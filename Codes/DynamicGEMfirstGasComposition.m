...
%% Dynamic GEM preparation
%Overal Results
vMch4=[vMch4_1;vMch4_2;vMch4_3;vMch4_4;vMch4_5;vMch4_6];
vMco2=[vMco2_1;vMco2_2;vMco2_3;vMco2_4;vMco2_5;vMco2_6];
vMo2=[vMo2_1;vMo2_2;vMo2_3;vMo2_4;vMo2_5;vMo2_6];
vPco2=[vPco2_1;vPco2_2;vPco2_3;vPco2_4;vPco2_5;vPco2_6];
vPo2=[vPo2_1;vPo2_2;vPo2_3;vPo2_4;vPo2_5;vPo2_6];

% Output the model results based on coculture growth:
% Please insert the time points:% 
timeGEM=[6,8,14,20,26,32,38,42,50,58,66,74,82,90]; %hours
save('dataforGEM1','timeGEM');

%DataGEM:% [time, photautotroph biomass, methanotroph biomass,vMch4,vMco2,vMo2,vPco2,vPo2]
DataGEM = [T,Y(:,1),Y(:,2),vMch4,vMco2,vMo2,vPco2,vPo2];
save('dataforGEM2','DataGEM');

% yeild calculation %
conMch4ave =0;
conPco2ave =0;

feedingCycle = 6; %number of feeding cycles
for i=1:feedingCycle 
conMch4ave = conMch4{1,N}(i)+ conMch4ave;
conPco2ave = conPco2{1,N}(i)+ conPco2ave;
end
%yields (gDCW/mmol)
YXmCH4ave = Y(end,2)*Vl/conMch4ave;
YXpCO2ave = Y(end,1)*Vl/conPco2ave;
%total uptake rate (mmol/hr):
vMch4tot = vMch4.*Y(:,2)*Vl;
vPco2tot = vPco2.*Y(:,1)*Vl;
vMo2tot = vMo2.*Y(:,2)*Vl;
vPo2tot = vPo2.*Y(:,1)*Vl;
...
% plot for uptake rates
figure
plot(T,vMch4,T,vPco2)%
ylabel('uptake rate (mmol/gDCW/hr)')
yyaxis right
plot(T,vMch4tot,T,vPco2tot)%
ylabel('total uptake rate (mmol/hr)')
ylabel('uptake rate (mmol/gDCW/hr)')
hold on
for iii=1:length(timeGEM)
    xline(timeGEM(iii),'LineWidth',1);
end